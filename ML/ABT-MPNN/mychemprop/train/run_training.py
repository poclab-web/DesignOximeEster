...
from mychemprop.args import TrainArgs
from mychemprop.constants import MODEL_FILE_NAME
from mychemprop.data import get_class_sizes, get_data, MoleculeDataLoader, MoleculeDataset, set_cache_graph, split_data
from mychemprop.models import MoleculeModel
from mychemprop.nn_utils import param_count
from mychemprop.utils import build_optimizer, build_lr_scheduler, get_loss_func, load_checkpoint, makedirs, \
...
def run_training(args: TrainArgs,
    ...
    :param args: A :class:`~mychemprop.args.TrainArgs` object containing arguments for
                 loading data and training the Chemprop model.
    :param data: A :class:`~mychemprop.data.MoleculeDataset` containing the data.
    ...
    if args.save_preds:
        # test_smiles = [x[2:-2] for x in test_data.smiles()]
        test_smiles = [s[0] for s in test_data.smiles()]
        test_preds_dataframe = pd.DataFrame(
            data={'smiles': test_smiles})

        for i, task_name in enumerate(args.task_names):
            test_preds_dataframe[task_name + "_target"] = [pred[i]
                                               for pred in test_data.targets()]
            test_preds_dataframe[task_name + "_pred"] = [pred[i] for pred in avg_test_preds]
