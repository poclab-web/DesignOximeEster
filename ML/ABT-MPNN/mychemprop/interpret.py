...
from mychemprop.args import InterpretArgs
from mychemprop.data import get_data_from_smiles, get_header, get_smiles, MoleculeDataLoader, MoleculeDataset
from mychemprop.train import predict
from mychemprop.utils import load_args, load_checkpoint, load_scalers, timeit
...
class ChempropModel:
    """A :class:`ChempropModel` is a wrapper around a :class:`~mychemprop.models.model.MoleculeModel` for interpretation."""

    def __init__(self, args: InterpretArgs) -> None:
        """
        :param args: A :class:`~mychemprop.args.InterpretArgs` object containing arguments for interpretation.
        """
        self.args = args
...
def interpret(args: InterpretArgs) -> None:
    ...
    mychemprop_model = ChempropModel(args)

    def scoring_function(smiles: List[str]) -> List[float]:
        return mychemprop_model(smiles)[:, args.property_id - 1]
...
def mychemprop_interpret() -> None:
    """Runs interpretation of a Chemprop model.

    This is the entry point for the command line command :code:`mychemprop_interpret`.
    """
