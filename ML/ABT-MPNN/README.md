...
構築したら環境構築したpythonパスをsetting.jsonに記載する
...
```
csv_path=$1 <- csvのパス
save_dir=$2 <- 保存先ディレクトリ
smiles_column=$3 <- csvファイルのsmilesカラム名
task_name=$4 <- タスク名
```

`task_name`はログの出力場所を指定する。`task_name="sample"`とした場合、`log/sample`にログが出力される
...
サンプルデータのインストール: https://drive.google.com/file/d/1reNIj65nKHPblIM00_gpbQqmWrmNZc1k/view?usp=drive_link
