...
def get_stats(train_set: Dataset,
    ...
    try:
        ...
        with open(f'{generated_path}/stats.json', 'w') as f:
            json.dump(stats, f, default=int64_to_int)
    ...
    try:
        ...
        print('error whe save dataframe', e)
        print('data', data)
        print(type(data))
        save_data = {**{'Smiles': valid_generated_smiles}}
        df = pd.DataFrame(save_data)
        df.to_csv(f'{generated_path}/generated_smiles.csv', index=False)
        # alternate save json
        # with open(f'{generated_path}/generated_smiles.json', 'w') as f:
        #     json.dump(data, f)
