...
def main(token_json_path, dataset_path):
    tokenizer = BPETokenizer(token_json_path, dataset_path, 500)

    print(tokenizer.vocab_size)
    smiles = '[BOS]C=CC12CC(C#N)C(=O)C1C(=O)O2'
    smiles = r"[BOS]CC(=O)O\N=C(C)\C(=O)c(cc1)cc(c12)C(CCCC)(CCCC)c3c2ccc(c3)C(=O)/C(=N\OC(=O)C)c4ccccc4"
    encodings = tokenizer(smiles, max_length=300, padding=True)
    print(encodings)
    decoded = tokenizer.decode(encodings['input_ids'])
    print(decoded)

if __name__ == "__main__":
    token_json_path = "/mnt/ssd2/Chem/photopolymerization_initiator/ML/MolGen/data/tokenizers/zincBPETokenizer_new.json"
    dataset_path="/mnt/ssd2/Chem/photopolymerization_initiator/ML/MolGen/data/mydata/pubchem_aldrich_filter.smi"
    main(token_json_path, dataset_path)
